...
#if defined(NDEBUG)
# error "Tpccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0575f8f3da923529adce1ab9f73d3358aef67b1ff13b1971ab6e2e0581fb6eb8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tpccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tpccoin Signed Message:\n";
...
{
    // Tpccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Tpccoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // Tpccoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tpccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tpccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
       pchMessageStart[0] = 0xc1;
        ...
        hashGenesisBlock = uint256("0x0575f8f3da923529adce1ab9f73d3358aef67b1ff13b1971ab6e2e0581fb6eb8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "TPC Coin , The Public Coin , December 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("t1234040184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514975610;
        ...
        assert(block.hashMerkleRoot == uint256("0xb20b414c2d0792cdf87b6ff766902540237dc844f737af12484a538d577430ba"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Tpccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TpccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TpccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TpccoinMiner(CWallet *pwallet)
{
    printf("TpccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tpccoin-miner");
    ...
    try { loop {
        ...
        printf("Running TpccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TpccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TpccoinMiner, pwallet));
}
